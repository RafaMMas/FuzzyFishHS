...
if(!is.null(data))
{
h <- hist(data[which(data[,"Species"]==0),i], breaks = seq(0, max(DB[,i]), length=ifelse(i=="Cover.index", 7, 10)) , plot=F)
h$counts <- h$counts / sum(h$counts)
plot(h, freq=TRUE, col=adjustcolor("grey", alpha.f=0.5), border="grey30", ylim=c(0, 1), xlab="", ylab="", main="",axes=F);par(new=T)

h <- hist(data[which(data[,"Species"]==1),i], breaks = seq(0, max(DB[,i]), length=ifelse(i=="Cover.index", 7, 10)) , plot=F)
h$counts <- h$counts / sum(h$counts)
plot(h, freq=TRUE, col=adjustcolor("deeppink", alpha.f=0.35), border="deeppink4", ylim=c(0, 1), xlab="", ylab="", main="",axes=F);par(new=T)
